    ...
    return (
      <div className="flex flex-col gap-6">
        <SectionTitle
          ...
          description="Interactive map of monitoring stations and water bodies."
          right={<StatusPill tone="good">{totalStations} stations</StatusPill>}
        />
        <GlassCard className="p-0 overflow-hidden" delay={0}>
          <div className="flex flex-wrap items-center justify-between gap-3 border-b border-white/[0.06] p-4">
            <div className="flex items-center gap-2 text-sm">
              <MapIcon className="h-4 w-4 text-cyan" /> HydroVision Network · Bengaluru cluster
            </div>
          </div>
          <div
            style={{ height: 520 }}
  ...
  return (
    <div className="flex flex-col gap-6">
      ...
      <GlassCard className="p-0 overflow-hidden" delay={0}>
        <div className="flex flex-wrap items-center justify-between gap-3 border-b border-white/[0.06] p-4">
          <div className="flex items-center gap-2 text-sm">
            <MapIcon className="h-4 w-4 text-cyan" /> HydroVision Network · Live Tracking
          </div>
          <div className="flex flex-wrap gap-2">
            ...
            {["Stations", "Heat map", "Pollution zones", "Satellite", "Rivers", "Lakes"].map(
              (l, i) => (
                <button
                  key={l}
                  className="inline-flex items-center gap-1.5 rounded-full border border-white/10 text-muted-foreground px-2.5 py-1 text-[11px] transition hover:text-foreground"
                >
              ...
              >
                <Popup>
                  <div style={{ fontFamily: "inherit", minWidth: 180 }}>
                    <div style={{ fontWeight: 600, marginBottom: 4 }} className="text-cyan">
                      ESP32-S3 Node (Live)
                    </div>
                    <div style={{ fontSize: 11, opacity: 0.7, marginBottom: 6 }}>
                      Coordinates: {deviceLat.toFixed(6)}, {deviceLng.toFixed(6)}
                    </div>
                    <div
                      style={{
                        display: "grid",
                    ...
                    >
                      ...
                      <div>pH</div>
                      <div className="font-semibold">{current.ph}</div>
                      <div>TDS</div>
                      <div className="font-semibold">{current.tds} ppm</div>
                      <div>NTU</div>
                      <div className="font-semibold">{current.ntu}</div>
                      <div>WQI</div>
                      <div className="font-semibold text-emerald">{current.wqi}</div>
                      <div>Status</div>
                      <div className="font-semibold uppercase">{current.state || "GOOD"}</div>
                    </div>
                  </div>
                </Popup>
              </CircleMarker>
      ...
      <div className="grid gap-4 md:grid-cols-3">
        <ComingSoon
          ...
          description="Density heat map of contamination events across watersheds."
        />
        <ComingSoon
          tag="Coming Soon"
          title="Satellite Layer"
          description="Sentinel-2 true-color tiles with chlorophyll retrieval overlay."
        />
        <ComingSoon
          tag="Coming Soon"
          title="River & Lake Layer"
          description="Vector layers for hydrography and catchment areas."
        />
      </div>
    </div>
  );
