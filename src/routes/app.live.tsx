    ...
    <div className="flex flex-col gap-6">
          ...
          <GlassCard key={s.id} delay={i * 0.05}>
            <div className="flex items-start justify-between">
              ...
              <div className="flex items-center gap-2">
                <StatusPill
                  tone={
                    deviceStateLabel === "UNSAFE"
                      ? "bad"
                      : deviceStateLabel === "POOR"
                        ? "warn"
                        : "good"
                  }
                >
            ...
            <div className="mt-3 grid grid-cols-3 gap-2 text-[10px] uppercase tracking-wider text-muted-foreground">
              ...
              <div className="rounded-lg border border-emerald/20 bg-emerald/5 px-2 py-1.5 text-center text-emerald">
                Sensor OK
              </div>
            </div>
          </GlassCard>
      ...
      <div className="grid gap-4 md:grid-cols-3">
        <GlassCard className="flex flex-col justify-between">
          <div>
            ...
            <div className="mt-3 font-mono text-lg text-foreground">
              {current.latitude ? current.latitude.toFixed(6) : "0.000000"},{" "}
              {current.longitude ? current.longitude.toFixed(6) : "0.000000"}
            </div>
            <div className="text-xs text-muted-foreground">
              Fix Quality:{" "}
              {current.satellites && current.satellites >= 3 ? "3D Lock" : "No GPS Lock"}
            </div>
          </div>
          <button
        ...
        <GlassCard>
          ...
          <div className="mt-3 space-y-1.5 text-xs text-foreground/90">
            ...
            <div className="flex justify-between items-center">
              ...
              <StatusPill tone={isConnected ? "good" : "bad"}>
            ...
            <div className="flex justify-between items-center">
              ...
              <span className="font-mono text-cyan">
                {current.wifiStrength ? `${current.wifiStrength} dBm` : "Offline"}
              </span>
            </div>
            <div className="flex justify-between items-center">
        ...
        <ComingSoon
          ...
          title="Battery & Solar Status"
          description="Battery telemetry and solar harvesting metrics activate automatically when edge sensors publish to Firebase."
        />
      </div>
    </div>
  );
