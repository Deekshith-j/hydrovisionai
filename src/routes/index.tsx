  ...
  head: () => ({
    meta: [
      { title: "HydroVision AI — AI-Powered Water Intelligence Platform" },
      {
        name: "description",
        content:
          "Predicting tomorrow's water quality today. Real-time IoT sensing, AI prediction, and geospatial intelligence for water bodies.",
      },
...
function Background() {
  return (
    <>
      ...
      {/* Wave */}
      <svg
        className="pointer-events-none absolute inset-x-0 bottom-0 h-[280px] w-[200%] animate-wave text-cyan/10"
        viewBox="0 0 1440 320"
        preserveAspectRatio="none"
      >
        <path
          fill="currentColor"
          d="M0,192L48,181.3C96,171,192,149,288,160C384,171,480,213,576,208C672,203,768,149,864,144C960,139,1056,181,1152,181.3C1248,181,1344,139,1392,117.3L1440,96L1440,320L0,320Z"
        />
      </svg>
      {/* Particles */}
      {[...Array(14)].map((_, i) => (
...
function Nav() {
  return (
    <header className="relative z-20 mx-auto flex max-w-7xl items-center justify-between px-6 py-6">
      ...
      <Link
        to="/app"
        className="group inline-flex items-center gap-1.5 rounded-full border border-white/10 bg-white/[0.04] px-4 py-2 text-sm font-medium transition hover:border-cyan/40 hover:bg-white/[0.07]"
      >
        Launch console
    ...
    <section className="relative z-10 mx-auto max-w-7xl px-6 pb-24 pt-16 sm:pt-24">
        ...
        <div className="glass rounded-3xl p-3 shadow-[var(--shadow-elevated)]">
          <div className="grid grid-cols-2 gap-3 md:grid-cols-4">
            {[
              ...
              >
                ...
                <div className={`mt-2 font-display text-2xl font-semibold ${s.tone}`}>
                  {s.value}
                </div>
              </div>
            ))}
          ...
          <div className="mt-3 flex items-center justify-between rounded-2xl border border-white/[0.06] bg-white/[0.02] px-4 py-3 text-xs text-muted-foreground">
            <span className="flex items-center gap-2">
              <span className="h-1.5 w-1.5 animate-pulse rounded-full bg-emerald" /> All systems
              operational
            </span>
      ...
      <div className="mt-16 flex justify-center text-muted-foreground">
        <motion.div
          ...
          className="flex flex-col items-center gap-1 text-[11px] uppercase tracking-[0.3em]"
        >
          ...
          {[
            {
              icon: Cpu,
              title: "IoT Sensing",
              desc: "ESP32 nodes streaming pH, TDS, turbidity, temperature and GPS.",
            },
            {
              icon: Database,
              title: "Realtime Backbone",
              desc: "Firebase Realtime Database ingest with millisecond fan-out.",
            },
            {
              icon: Sparkles,
              title: "AI Prediction",
              desc: "Risk scoring, algal bloom forecasting, 24/48/72h horizons.",
            },
            {
              icon: Map,
              title: "Geo Intelligence",
              desc: "Interactive Leaflet maps with pollution and water-quality layers.",
            },
          ].map((f, i) => (
            <motion.div
              key={f.title}
  ...
  const items = [
    {
      ...
      desc: "Every sensor, streamed live with historical comparison and health metadata.",
      tag: "Realtime",
    },
    {
      title: "AI Prediction Engine",
      desc: "Random Forest, XGBoost, and LSTM ensembles power multi-horizon forecasts.",
      tag: "ML",
    },
    {
      title: "Algal Bloom Radar",
      desc: "Dedicated harmful bloom module with contributing factors and timelines.",
      tag: "Predictive",
    },
    {
      title: "Historical Analytics",
      desc: "Beautiful daily, weekly, monthly, yearly charts with CSV/Excel/PDF export.",
      tag: "Insights",
    },
    {
      title: "Geo Intelligence",
      desc: "Leaflet-powered maps with heat, pollution, satellite, river and lake layers.",
      tag: "Spatial",
    },
    {
      title: "Environmental Insights",
      desc: "Lake health scoring, contamination timelines, seasonal trend detection.",
      tag: "Ecology",
    },
    {
      title: "Satellite Integration",
      desc: "NASA Landsat, Sentinel-2 and MODIS — chlorophyll and surface temperature.",
      tag: "Coming soon",
      soon: true,
    },
    {
      title: "Digital Twin",
      desc: "Lake simulation and water dynamics for scenario planning and forecasting.",
      tag: "Coming soon",
      soon: true,
    },
  ];
    ...
    <section id="capabilities" className="relative z-10 mx-auto max-w-7xl px-6 py-16">
      <div className="mb-10 flex flex-wrap items-end justify-between gap-3">
        <div>
          ...
          <h2 className="mt-3 font-display text-3xl font-semibold tracking-tight sm:text-4xl">
            A complete water intelligence stack.
          </h2>
        </div>
        <Link
          to="/app"
      ...
      <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
        {items.map((it, i) => (
          ...
          >
            <div
              className={`inline-flex items-center rounded-full border px-2 py-0.5 text-[10px] uppercase tracking-widest ${it.soon ? "border-white/10 text-muted-foreground" : "border-cyan/25 text-cyan"}`}
            >
              {it.tag}
            </div>
    ...
    <footer id="roadmap" className="relative z-10 border-t border-white/5 py-10">
      <div className="mx-auto flex max-w-7xl flex-wrap items-center justify-between gap-3 px-6 text-xs text-muted-foreground">
        <div className="flex items-center gap-2">
          ...
          <span>
            © {new Date().getFullYear()} HydroVision AI · Environmental Intelligence Platform
          </span>
        </div>
        <div className="flex items-center gap-4">
          <Link to="/app/roadmap" className="hover:text-foreground">
            Roadmap
          </Link>
          <Link to="/app/about" className="hover:text-foreground">
            About
          </Link>
